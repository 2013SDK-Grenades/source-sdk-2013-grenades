...
CTFPlayer::CTFPlayer() 
{
	...
	m_flNextDenySound = 0.0f;	// FF Grenade Port
...
void CTFPlayer::PreThink()
{
	...
	// FF Grenade Port: grenade button input is now handled in ItemPostFrame()
	// via the SetOffHandWeapon() off-hand path (see tf_player_shared.cpp).
...
void CTFPlayer::GiveDefaultItems()
{
	...
	if ( !TFGameRules() || !TFGameRules()->IsInMedievalMode() )
	{
		...
		ManageGrenades( pData );	// FF Grenade Port: give class-appropriate grenade weapons
	}
	...
	if ( ff_grenades.GetBool() && !IsPrimed() && CanAttack() && bCanThrow && pData )
	{
		for ( int iGrenade = 0; iGrenade < TF_PLAYER_GRENADE_COUNT; iGrenade++ )
		{
			if ( m_afButtonPressed & ( iGrenade == 0 ? IN_GRENADE1 : IN_GRENADE2 ) )
			{
				...
				{
					// FF Grenade Port: check ammo before priming — no ammo, play deny sound.
					if ( GetAmmoCount( pGrenade->GetPrimaryAmmoType() ) <= 0 )
					{
