...
void CFFCaltrop::CaltropTouch ( CBaseEntity *pOther )
{
	...
	if( pPlayer && !pPlayer->IsObserver() )
	{
		if( !pPlayer->InSameTeam( GetOwnerEntity() ) )  // FF Grenade Port: FCanTakeDamage->InSameTeam
		{
			...
			float flSpeed = 0.8f;
			// FF Grenade Port: was `if( pPlayer->LuaRunEffect( SE_LEGSHOT, this, &flDuration,
			// &flIconDuration, &flSpeed ) )`. LuaRunEffect() was FF's Lua-scripting hook (always
			// returns true with no scripts loaded). Lua removed project-wide; stripped here too.
			{
				...
				// FF Grenade Port: was pPlayer->AddSpeedEffect( SE_CALTROP, flDuration, flSpeed,
				// SEM_ACCUMULATIVE | SEM_HEALABLE, FF_STATUSICON_CALTROPPED, flIconDuration ) --
				// FF's generic multi-slot speed-effects framework (m_vSpeedEffects[], used across
				// caltrops/tranq darts/etc, supporting multiple simultaneous stacking effects).
				// Not being ported -- too broad for one grenade type. Using TF2's native
				// GetLaggedMovementValue()/SetLaggedMovementValue() directly instead, the same
				// mechanism the slowfield grenade uses. Trade-off: no stacking with other speed
				// effects (e.g. simultaneous slowfield + caltrop just takes the most recent
				// value, rather than FF's accumulative blend), and no HUD status icon (deferred
				// along with other assets/scripts work).
				pPlayer->SetLaggedMovementValue( flSpeed );

				// caltrop slow has no explicit "wears off" callback like the slowfield grenade's
				// per-think proximity check -- timer/removal left as a known gap, noted in the
				// final summary rather than guessed at here.
			}
		}		
	}
}
